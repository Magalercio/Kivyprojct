...
class BannerVenda(GridLayout):
    ...
    def __init__(self, **kwargs):  # Faz com que tudo abaixo ocorra na inicialização da classe
        self.rows = 1  # definindo quantidade de linhas do GridLayout
        super().__init__()  # chama os parametros iniciais do GridLayout pra depois adcionar tudo abaixo.

        with self.canvas:  # criando o fundo preto
            Color(rgb=(0, 0, 0, 1))
        ...
        direita = FloatLayout()
        direita_label_data = Label(text=f"Data: {data}", pos_hint={"right": 1, "top": 0.90}, size_hint=(1, 0.33))
        direita_label_preco = Label(text=f"Preço: R$ {preco:,.2f}", pos_hint={"right": 1, "top": 0.65},
                                    size_hint=(1, 0.33))
        direito_label_quantidade = Label(text=f"{quantidade} {unidade}", pos_hint={"right": 1, "top": 0.40},
                                         size_hint=(1, 0.33))
        direita.add_widget(direita_label_data)
        direita.add_widget(direita_label_preco)
        direita.add_widget(direito_label_quantidade)
